...
public class AvcEncoder {
    ...
    private int mImageFormat = 0;//摄像头采集格式
    private int mColorFormat = 0;//编码器颜色格式
...
//        mediaFormat.setInteger(MediaFormat.KEY_COLOR_FORMAT, MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420SemiPlanar);// 设置颜色格式，京东平板不支持
//        mediaFormat.setInteger(MediaFormat.KEY_COLOR_FORMAT, MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420Planar);// 设置颜色格式
//        mediaFormat.setInteger(MediaFormat.KEY_COLOR_FORMAT, MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420Flexible);// 设置颜色格式对应camera YUV_420_888格式
        mColorFormat = selectColorFormat(selectCodec(MIME_TYPE), MIME_TYPE);
        Log.v(TAG, "encoder mColorFormat=" + (mColorFormat == MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420Planar ? "yuv420p"
        ...
        mediaFormat.setInteger(MediaFormat.KEY_I_FRAME_INTERVAL, 5);// 设置关键帧间隔时间
...
//                                      swapYV12ToI420(input, yuv420p, m_width, m_height);//京东平板使用
                                        break;
                                }
                                break;
                        ...
                        input = yuv420;
                        Log.v(TAG, "convertDataLen=" + input.length);
                    }
                    if (input != null) {
                        try {
    ...
    */
    ...
    public static int selectColorFormat(MediaCodecInfo codecInfo, String mimeType) {
        Log.v(TAG, "codecInfo: name=" + codecInfo.getName());
